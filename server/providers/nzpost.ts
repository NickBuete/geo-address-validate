...
const NZPOST_BASE_URL = (process.env.NZPOST_BASE_URL ||
  'https://api.nzpost.co.nz/addresschecker/1.0'
).replace(/\/$/, '')
...
async function buildNzPostHeaders(forceRefresh = false): Promise<HeaderResult | null> {
...
function mapNzPostSuggestion(address: any, index: number) {
  ...
  const source = address?.SourceDesc || address?.source_desc || address?.source || address?.type
...
async function nzPostAustraliaDetail(id: string): Promise<DetailResponse> {
  ...
  const data = await requestNzPostJson(url)
  const address = data?.address ?? data?.addresses?.[0] ?? data?.details?.[0] ?? data
  if (!address) {
