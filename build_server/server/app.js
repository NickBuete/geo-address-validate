...
const providerAliases = {
    ...
    AU: nzpost_1.nzPostAustraliaProvider.id,
    AUS: nzpost_1.nzPostAustraliaProvider.id,
    AUSTRALIA: nzpost_1.nzPostAustraliaProvider.id,
...
app.get('/api/predict', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    ...
    try {
        const result = yield provider.predict(searchValue);
        res.json(result);
    }
    catch (error) {
...
app.get('/api/address/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!provider)
        return res.status(400).json({ message: `Unknown provider: ${providerId}` });
    try {
        const result = yield provider.detail(id);
        res.json(result);
    }
    catch (error) {
        handleProviderError(res, error);
